...
const MemoryGame = () => {
  const { game, gameState, onCardClickHandler, filppedCardsIdx, startOver } =
    useMemoryGame();
  return (
...
const Cell = ({ idx, onCardClickHandler, emoji, filppedCardsIdx }: Props) => {
  ...
  if (emoji === "")
    return (
      <div className="h-32 w-32 border border-gray-300 flex justify-center items-center" />
    );
  return (
    <div
      onClick={onClick}
      key={emoji}
      className="h-32 w-32 border border-gray-300 flex justify-center items-center"
    >
      {filppedCardsIdx.indexOf(idx) > -1 ? (
        <div
          className="text-[6rem] flex justify-center items-center"
